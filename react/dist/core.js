...
function paint(nodes, variant, mode, strokeWidth) {
    const result = [];
    nodes.forEach((node, index) => {
        if (node.tag === "g") {
            const children = paint(node.children, variant, mode, strokeWidth);
            if (children.length)
                result.push(h("g", { ...node.attrs, key: index }, ...children));
            return;
        }
        const attributes = { ...node.attrs, key: index };
        if (mode === "mask") {
            if (node.role !== "cut" && node.role !== "accent")
                return;
            Object.assign(attributes, { fill: "none", stroke: "black", strokeWidth: strokeWidth * (1.75 / 2.1) });
        }
        else {
            if (variant === "solid" && node.role === "cut")
                return;
            ...
            if (node.role === "body" || node.role === "accent") {
                ...
                if (variant === "duotone")
                    Object.assign(attributes, { fill: "currentColor", fillOpacity: node.role === "body" ? 0.14 : 0.30 });
            }
            if (node.role === "line" && variant === "solid")
                attributes.strokeWidth = strokeWidth;
            if (node.role === "dot")
                Object.assign(attributes, { fill: "currentColor", stroke: "none" });
        }
...
export function createClinicalIcon(slug, displayName, nodes, beamPath) {
    const Icon = forwardRef(function ClinicalIcon(props, ref) {
        const { size = 48, width, height, variant = "outline", strokeWidth = 2.1, animation = "none", duration = 2.6, beamColor, title, ariaLabel, "aria-label": explicitLabel, "aria-hidden": explicitHidden, style, ...rest } = props;
        const instanceId = useId();
        const maskId = "hci-" + slug + "-" + instanceId;
        const activeVariant = variant === "filled" ? "solid" : variant;
        ...
        if (activeVariant === "solid") {
            ...
            if (cutouts.length) {
                art = [
                    h("defs", { key: "defs" }, h("mask", { id: maskId, maskUnits: "userSpaceOnUse", x: 0, y: 0, width: 48, height: 48, style: { maskType: "luminance" } }, h("rect", { width: 48, height: 48, fill: "white" }), ...cutouts)),
        ...
        if (isBeam) {
            art = [
                ...
                h("g", { key: "beam", fill: "none", stroke: beamColor ?? "var(--hmi-beam-color, currentColor)" }, h("path", { className: "hmi-beam-tail", d: beamPath, pathLength: 100, strokeWidth: actualStroke * (3.8 / 2.1) }), h("path", { className: "hmi-beam-head", d: beamPath, pathLength: 100, strokeWidth: actualStroke * (2.3 / 2.1) })),
            ];
        }
